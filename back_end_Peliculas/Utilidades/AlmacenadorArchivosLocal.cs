...
{
    public class AlmacenadorArchivosLocal : IAlmacenadorArchivos // implentar interface
    {
        private readonly IWebHostEnvironment env;
        private readonly IHttpContextAccessor httpContextAccessor;

        public AlmacenadorArchivosLocal(IWebHostEnvironment env, IHttpContextAccessor httpContextAccessor) //asignar como un campo
        {
            this.env = env;
        ...
        public async Task<string> GuardarArchivo(string contenedor, IFormFile archivo)
        {
            ...
            if (!Directory.Exists(folder)) // si no existe directorio lo creo
            {
                Directory.CreateDirectory(folder);
            }

            string ruta = Path.Combine(folder, nombreArchivo);
            using (var memmoryStream = new MemoryStream())// arreglo de byte
            {
                await archivo.CopyToAsync(memmoryStream);
